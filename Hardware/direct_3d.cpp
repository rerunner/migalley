...
// This file contains WIN3D.CPP code related to direct3d, ported to vulkan
#include "_HARD.H"
...
#include	"HDATA.H"
#include	"HARDPASM.H"
#include	"WIN3D.H" // includes "direct_3d.h" as well
#include	"GAMESET.H"
#include	"TILEMAKE.H"
#include "AREATYPE.H"
#include "PROF.H"
...
extern void GreyPalette(UByte* rgb);
extern void GreyPalette(UByte* r,UByte* g,UByte* b);
...
{
	int iang = ang & 0xFFFF; // mask to 16 bits
    double radians = static_cast<double>(iang) * (M_PI / 32768.0);
    tanAng = std::tan(radians);
}

extern void ZeroMem(void* pp, ULong ll);
...
//	if (gameSettings.m_bUseMipMapping)
	if (Save_Data.filtering>=2)
	{
		...
		{
			mipMapLevels--;
			//RERUN if (mipMapLevels=-1) mipMapLevels=0;
			if (mipMapLevels < 0) mipMapLevels = 0; //RERUN
		}
	}
	else mipMapLevels=0;
}
...
//
//------------------------------------------------------------------------------
void DDErrorCodes(const char* file,int lineno,HRESULT dderr)
{
...
//
//------------------------------------------------------------------------------
void direct_3d::SetViewParams(ANGLES fov, MigWindow* w,DirectDP lpdirectd)
{
	Float	temp;
...
{
	DWORD	dwFree = 67108864; // 64MB default
#if 0
	HRESULT	dderrval;
	DDSCAPS	ddsCaps;
	DWORD	dwTotal;
...
//
//------------------------------------------------------------------------------
void direct_3d::TestHW3D(struct _DirectDraw* lpdirectd,MigWindow*)
{
}
...
{
	ULong textureRAMavail =  134217728; // RERUN 128 MB or so
#if 0
	struct TextureRec
	{
	...
	textureRAMavail=	texturesAllocated*
							_textureWH*
							_textureWH*
