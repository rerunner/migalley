...
#include "MIGVIEW.H"
#include "NODE.H"
#include "WORLDINC.H" //RERUN to include MILES.H
#include "MILES.H" //RERUN for _Miles
...
#include "TITLEBAR.H" //RERUN
#include "MAINTBAR.H" //RERUN see below
#include "MAINFRM.H" //RERUN combination with MAINTBAR for MainToolBar
#include "MISSMAN2.H" //RERUN for MMC
#include "PROFILE.H" //RERUN
#include "MISSFLDR.H" //RERUN for CMissionFolder
#include "PERSONS2.H" //RERUN
#include "MYMATH.H" //RERUN for Math_Lib
...
//		DialList(DialBox(FIL_NULL,
	...
	switch (p->MigStatus.LaunchFlags.size)
	{
	case WAYPOINTSIZE:
		{
			int pack, wave, g;
					...
					for (g = 0; g < 3; g++)
					{
						...
						if (ag)
							if (!(Todays_Packages[pack].CalcFuel(wave,g,UID_NULL, fuel)))
								RDialog::RMessageBox(IDS_BINGOWARNINGTITLE, IDS_BINGOWARNING,0,IDS_OK);
					}
...
BOOL CMapDlg::OnEraseBkgnd(CDC* pDC) 
{
	...
	// RERUN: Draw icons on top of the map
	...
	for (int i=UID_Null;i<WayPointBAND;i++)
	{
		...
		if (p)
		{
			...
			if (f)
			{
				RDialog::m_pView->ScreenXY(p, px, py, 0, 0);
				RDialog::m_pView->DrawIcon(pDC->GetSafeHdc(), px, py, f);
			}
		}
	}
...
{	//waypoint is somewhere in there...
		px=std::min<int>(ox,px);
		py=std::min<int>(oy,py);
...
void CMapDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (m_LButtonDown && !m_bDragging && m_buttonid)
	{
			...
			if (FindMapItem(m_point) && AllowDragItem(m_buttonid))
		{
			...
			m_hintid=0; // dont display hints while dragging...
		}
	}
	if (m_LButtonDown && m_bDragging && m_buttonid)
	{
