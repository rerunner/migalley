...
//	spinbut->AddString("5");
	...
	rlistbox->SetFontNum(2); // RERUN: Use Font 2 (11pt) for this listbox
	rlistbox->Clear();
	...
	// RERUN: Set header row color to Light Blue (Cyan)
	rlistbox->SetRowColour(0, RGB(0, 255, 255));

	//sum to get all column
	for (int i = Debrief::SORTIES; i <= Debrief::LOSSES; i++)
	{
		int kills = 0;
		for (int j = SQ_F86A; j < (SQ_B_NONFLYABLE); j++)
		{
			kills += MMC.debrief.playertotals[static_cast<SquadSelect>(j)][static_cast<Debrief::PlayerTypes>(i)];
		}
		MMC.debrief.playertotals[SQ_B_NONFLYABLE][static_cast<Debrief::PlayerTypes>(i)] = kills;
	}
	...
	for (int i = Debrief::SORTIES; i <= Debrief::LOSSES; i++)
	{
		for (int j = SQ_F86A; j < (SQ_B_NONFLYABLE+1); j++)
		{
			int kill = MMC.debrief.playertotals[static_cast<SquadSelect>(j)][static_cast<Debrief::PlayerTypes>(i)];
