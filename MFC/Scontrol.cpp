...
void	SController::RemakeDisplay()
{
	...
	if (index==-1 || index >= useddevices) index=0; // RERUN: Safety check
    
    if (useddevices == 0) return; // Prevent access to empty arrays

	combo->Clear();
	for (int i=0;i<useddevices;i++)
		combo->AddString(CSprintf("%s: %s",connecteddevices[i].name.c_str(),connecteddevices[i].prodname.c_str()));	
	...
	for (int i=firstaxes;i<usedaxes;i++)
	{
		...
		if (paironly && !connectedaxes[i].ishat)
			combo->AddString(CSprintf("%s: %s & %s",
									connecteddevices[connectedaxes[i].devnum].name.c_str(),
									connectedaxes[i].name.c_str(),connectedaxes[i+1].name.c_str()));
		else
			combo->AddString(CSprintf("%s: %s",
									connecteddevices[connectedaxes[i].devnum].name.c_str(),
									connectedaxes[i].name.c_str()			));
		currindex++;
	}
