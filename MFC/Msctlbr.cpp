...
#include "REPLAY.H" //RERUN for _Replay
...
void CMiscToolbar::OnClickedReplay() 
{
	...
	if (MMC.indebrief)
	{
		if (_Replay.badrecord)									//AMM 09Mar99
			RFullPanelDial::RMessageBox(IDS_MIGALLEY,IDS_BADREPLAY,0,IDS_OK);//AMM 09Mar99
		else
		{
...
void CMiscToolbar::OnClickedThumbnail() 
{
	...
	if (!LoggedChild(THUMBNAIL))
	{
		...
		CRect rect = Place(POSN_MIN,POSN_MIN);
		Edges edges(EDGE::ALIGN_MIN + EDGE::ACTIONS_SPLITTER,EDGE::ALIGN_MIN,EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE,EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE);
		DialBox box(FIL_NULL,CMIGView::m_thumbnail,edges);
		LogChild(THUMBNAIL,
			RDialog::MakeTopDialog(rect, box)
		);
	}
#else
	if (!LoggedChild(THUMBNAIL))
	{
		CMIGView::m_thumbnail=new CThumbnail;
		CRect p = Place(POSN_MIN, POSN_MIN);
		Edges e(
			EDGE::ALIGN_MIN + EDGE::ACTIONS_SPLITTER,
			EDGE::ALIGN_MIN,
			EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE,
			EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE
		);
		DialBox d(FIL_NULL, CMIGView::m_thumbnail, e);
		auto* dlg = RDialog::MakeTopDialog(p, d);
		LogChild(THUMBNAIL, dlg);
	}
#endif
	else
...
//	CFiling::selectedfile="test.sav";
	...
	{
		CRect rect = Place(POSN_CENTRE,POSN_CENTRE);
		Edges fileEdges(EDGE::ALIGN_MIN,EDGE::ALIGN_MIN,EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE,EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE);
		DialBox fileBox(FIL_NULL,new CFiling,fileEdges);

		IdEntry idSave(IDS_SAVE);
		IdEntry idLoad(IDS_LOAD);
		IdList ids(idSave, idLoad);
		Edges tabEdges(EDGE::ALIGN_MIN,EDGE::ALIGN_MIN,EDGE::ALIGN_MAX,EDGE::ALIGN_MAX);
		
		DialBox saveBox(FIL_MAP_PLAYER_LOG,new CLoad(IDD_SAVE,LSD_SAVE,			FIL_SAVEGAMEDIR,wildcard,CFiling::selectedfile));
		DialBox loadBox(FIL_MAP_PLAYER_LOG,new CLoad(IDD_LOAD,LSD_LOAD_DESELECT,FIL_SAVEGAMEDIR,wildcard,CFiling::selectedfile));

		HTabBox tabs(FIL_NULL,ids,tabEdges,saveBox,loadBox);
		DialList list(fileBox, tabs);

		LogChild(FILES,
			RDialog::MakeTopDialog(rect, list)
		);
	}
	else
...
void CMiscToolbar::OpenSaveOnly(bool quitafter) 
{
	...
	if (!LoggedChild(FILES))
	{
		CRect rect = Place(POSN_CENTRE,POSN_CENTRE);
		Edges fileEdges(EDGE::ALIGN_MIN,EDGE::ALIGN_MIN,EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE,EDGE::ALIGN_MAX + EDGE::OCCLUDE_INSIDE);
		DialBox fileBox(FIL_NULL,new CFiling(true),fileEdges);

		IdEntry idQuitSave(IDS_QUITSAVE);
		IdList ids(idQuitSave);
		Edges tabEdges(EDGE::ALIGN_MIN,EDGE::ALIGN_MIN,EDGE::ALIGN_MAX,EDGE::ALIGN_MAX);
		
		DialBox saveBox(FIL_MAP_PLAYER_LOG,new CLoad(IDD_SAVE,LSD_SAVE,			FIL_SAVEGAMEDIR,wildcard,CFiling::selectedfile));

		HTabBox tabs(FIL_NULL,ids,tabEdges,saveBox);
		DialList list(fileBox, tabs);
