...
//#include "rdialog.h"
#include "MESSAGES.H"
#include "FILES.H" //RERUN always needed before FILEMAN.H
#include "FILEMAN.H"
...
#include "RBUTTON.H"
#include "RMDLDLG.H"
#include "SAVEGAME.H" // Needed before MAINFRM.H
...
void	RDialog::DDX_Control(CDataExchange* pDX,int	dialid,DlgItem&	m_dial)
{
	// RERUN: In the stub, we don't have real child windows created from templates yet.
	// We just want to bind the member variable so that GETDLGITEM works.
	// HWND tmp;
...
//		CView* view=frame->GetActiveView();
		if (tree->edges && (tree->edges->l&EDGE::ACTIONS_MASK)==EDGE::ACTIONS_ARTCHILD) // These are meant to be internal panels
			main->Create(IDD_EMPTYCHILD,m_pView); // Keep as child of m_pView
		else if (tree->edges && (tree->edges->l&EDGE::OCCLUDE_MASK)==EDGE::OCCLUDE_EXACT) // These are meant to be internal panels
			main->Create(IDD_EMPTYNOBORDER,m_pView); // Keep as child of m_pView
		else // Other top-level dialogs should be independent windows
			main->Create(main->MY_IID,NULL); // Pass NULL as parent to make it a top-level SDL window
	}
	// scale the dialog if necessary due to font size change
...
{
	std::cout << "[RDialog] EndDialog called on " << this << " Result=" << nResult << std::endl;
...
void RDialog::OnOK( )
{
	std::cout << "[RDialog] OnOK called on " << this << std::endl;
	if (edges.l&EDGE::ACTIONS_ARTCHILD) return;
...
//				ASSERT_KINDOF(CRSpltBar,splitterTest);
					...
					abs(splitterRect.left-dialRect.right)<seperation)
				{
					...
					if (splitterTest->GetTopLeftWindow()==0)
					{
		...
		if (splitter)
		{
			...
			TRACE0("Failed to find the left edge of an unused vertical splitter bar\n");
		//delete splitter;
	}
	seperation=INT_MAX;
	splitter=NULL;
...
//				ASSERT_KINDOF(CRSpltBar,splitterTest);
				...
				{
					if (splitterTest->GetBottomRightWindow()==0)
					{
...
static void PalTrans(void*& dataarea, ULong& datasize) { /* no-op */ }
...
void RDialog::OnPaint() 
{
	...
	if (artnum)
	{
		...
		if (pData && pData[0]=='B' && pData[1]=='M') // checks if its a bitmap file
		{
			...
			pDC->SetDIBitsToDevice(short(offsets & 0x0000FFFF),short((offsets & 0xFFFF0000)>>16),pInfo->bmiHeader.biWidth, pInfo->bmiHeader.biHeight,
				0,0,0,pInfo->bmiHeader.biHeight,pData,pInfo,DIB_RGB_COLORS);
		}
		else
		{
			// RERUN: Fallback drawing for dialogs if art load fails
			CRect rect;
			GetClientRect(rect);
			pDC->FillSolidRect(rect, RGB(192, 192, 192));
			pDC->Draw3dRect(rect, RGB(255, 255, 255), RGB(128, 128, 128));

			// RERUN: Add debug prints
			std::cout << "[RDialog::OnPaint] Fallback for DlgID: " << MY_IID << ", artnum: " << artnum << std::endl;
			if (pData) {
			...
			char buf[32];
			snprintf(buf, sizeof(buf), "Dlg:%d", MY_IID);
			pDC->SetTextColor(RGB(0, 0, 0));
			pDC->SetBkMode(TRANSPARENT);
			pDC->DrawText(buf, rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
		}
	}
	EndPaint(&paintstruct);
}
...
//				TRACE0("Y deviance detected\n");
			...
			TRACE1("Scrolling %d\n",m_hWnd);
		    ::SetCursor(nullptr /* RERUN AfxGetApp()->LoadStandardCursor(IDC_SIZEALL)*/);
			homesize.top-=dy;
			homesize.bottom-=dy;
			homesize.left-=dx;
			homesize.right-=dx;
...
//				ShowScrollBar(SB_VERT,TRUE);
				if (!m_pVertScrollBar)
				{
					...
					m_pVertScrollBar->Create(NULL, "", WS_CHILD | WS_VISIBLE, CRect(-1,-1,-1,-1), parent, IDJ_VERT_SCROLLBAR);
...
//				ShowScrollBar(SB_HORZ,TRUE);
				if (!m_pHorzScrollBar)
				{
					...
					m_pHorzScrollBar->Create(NULL, "", WS_CHILD | WS_VISIBLE, CRect(-1,-1,-1,-1), parent, IDJ_HORZ_SCROLLBAR);
...
//			cviewsize.bottom=cviewsize.top+child->minsize.Height();
	...
	if (tabcontrol!=NULL)
	{
		...
		tabcontrol->ShowWindow(SW_SHOW); // RERUN: Ensure tab control is visible
...
CFont* RDialog::OnGetGlobalFont(int fontnum)
{
    if (fontnum<0||fontnum>MAXFONTS)
		fontnum=0;
	CFont* f;
	if (m_scalingfactor>1.5) f = g_AllFonts[fontnum][3];
	else f = g_AllFonts[fontnum][0];
	
	if (!f) std::cout << "[RDialog] OnGetGlobalFont: Returning NULL for fontnum=" << fontnum << " scaling=" << m_scalingfactor << std::endl;
...
void RDialog::UpdateTitle()
{
	CWnd* title_wnd;
	CRect rect;
	title_wnd=GetDlgItem(IDJ_TITLE);
	if (title_wnd)
	{
		CRButton* title = dynamic_cast<CRButton*>(title_wnd);
		if (title)
		{
			// Designate this title bar button as a handle for moving the parent dialog.
			title->m_bMovesParent = TRUE;
		}

		title->GetClientRect(rect);
		int height = rect.Height();
		if (height < 10) height = 20; // RERUN: Enforce minimum title height

		// RERUN: Force title to top of client area (0,0)
		// The original logic using homesize/viewsize seems to misplace it off-screen (-650) in this environment.
		CRect clientRect;
		GetClientRect(clientRect);
		CRect newRect(0, 0, clientRect.Width(), height);
		// std::cout << "[RDialog] UpdateTitle: IDJ_TITLE Rect=" << newRect.left << "," << newRect.top << "," << newRect.right << "," << newRect.bottom << std::endl;
		
		title_wnd->MoveWindow(newRect);
	}
	//delete title;
}
