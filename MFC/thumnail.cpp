...
void CThumbnail::OnPaint() 
{
	...
	m_point.x=std::max<long>(m_boxSize.cx/2,m_point.x);
	m_point.y=std::max<long>((m_boxSize.cy/2)+rect.top,m_point.y);
	m_point.x=std::min<long>(rect.right-(m_boxSize.cx/2),m_point.x);
	m_point.y=std::min<long>(rect.bottom-(m_boxSize.cy/2),m_point.y);
	CPen pen;
