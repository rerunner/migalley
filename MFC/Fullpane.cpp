...
//RERUN #include "campback.h"
//RERUN #include "sairclms.h"
#include "GLOBREFS.H"
#include "PERSONS2.H"
//RERUN #include "load.h"
#include	"MILES.H"
#include	"BSTREAM.H"
#include "OVERLAY.H"
//RERUN #include	"credits.h"
...
#include "MAINTBAR.H" //RERUN for CMainFrame
#include "MAINFRM.H" //RERUN for CMainFrame, after MAINTBAR.H
#include "CREDITS.H" //RERUN for credits handling
#include "SDETAIL.H" //RERUN for CSDetail
#include "SGAME.H" //RERUN
#include "APILOT.H" //RERUN for CAutoPilot
#include "SVIEWER.H" //RERUN CSViewer
#include "SSOUND.H" //RERUN CSsound
#include "SFLIGHT.H" //RERUN CSFlight
#include "REDTBT.H" //RERUN for CREdtBt
#include "SCONTROL.H" //RERUN SController
#include "CAMPBACK.H" //RERUN for CCampBack
#include "SQUICK1.H"
#include "SQUICK2.H"
#include "SQUICKP.H"
#include "SQUICKUN.H" //RERUN for full declaration of CSQuickLine
#include "SQVIEW.H" //RERUN for CSQuickView
#include "VWQLIST.H" //RERUN for CSQuickList
#include "MTCHILD.H"
//#define QUICKQUICKQUICK //RERUN
#include "SQUICK1.H" //RERUN
#include "RTESTSH1.H" //RERUN
#include "SAIRCLMS.H" //RERUN for CSQuickAirclaims
#include "SGRDCLMS.H" //RERUN for CSQuickGroundClaims
#include "LOAD.H" //RERUN for CLoad
#include "SCAMP.H" //RERUN for CSCampaign
#include "LOCKER.H" //RERUN for CLockerRoom
#include "SERVICE.H" //RERUN for CSelectService
#include "SESSION.H" //RERUN for CSelectSession
#include "VISITORS.H" //RERUN for CVisitorsBook
#include "READY.H" //RERUN for CReadyRoom
#include "FRAGPILT.H" //RERUN for CFragPilot
#include "RADIO.H" //RERUN for CRadio
#include "VARIANTS.H" //RERUN for CVariants
#include "COMMSPNT.H" //RERUN for CCommSpaint
#include "CURREMBL.H" //RERUN for CCurrEmblem
#include "COMMSAC.H" //RERUN for CCommsDeathMatchAc
#include "CAMPWAR.H" //RERUN for CCampBackEntireWar
#include "FILING.H" //RERUN for CFiling
...
int RFullPanelDial::resolutions[]={640,800,1024,1280,1366,1920};
...
BEGIN_MESSAGE_MAP(RFullPanelDial, RDialog)
...
{ // launches the main dialog window
	...
	DialBox box(FIL_NULL, this, EDGES_NOSCROLLBARS_NODRAGGING);

	RDialog* dial=RDialog::MakeTopDialog(rect, box);
...
{
	CRListBox* prlistbox=GETDLGITEM(IDC_RLISTBOX);

	// RERUN: Ensure the listbox window is created so it can receive WM_PAINT
	if (prlistbox && !prlistbox->GetHwnd()) {
		CRect rect(0,0,100,100); // Will be resized/moved later in this function
		prlistbox->Create(NULL, "RListBox", WS_CHILD | WS_VISIBLE, rect, this, IDC_RLISTBOX);
	}

	// RERUN: Set font. Title menus screen uses font 6. 
	// Ideally this should be specified in the data for the screen, but for now just set it here.
	prlistbox->SetFontNum(6);
	...
	{	// a horizontal listbox...
		...
		for (int x=0;x<10;x++)
		{
			if (m_currentscreen->textlists[x].text==0) break;
	...
	else // a vertical one
	{
		...
		for (int x=0;x<10;x++)
		{
			if (m_currentscreen->textlists[x].text==0) break;
			CString thestring;
			thestring.LoadString(m_currentscreen->textlists[x].text);
...
Bool RFullPanelDial::QuickMissionBlue()
{
	...
	DialBox d0(FIL_NULL, (initind>0)?new CSQuickLine(0,ilun[0][0],ilun[0][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d1(FIL_NULL, (initind>1)?new CSQuickLine(0,ilun[1][0],ilun[1][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d2(FIL_NULL, (initind>2)?new CSQuickLine(0,ilun[2][0],ilun[2][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d3(FIL_NULL, (initind>3)?new CSQuickLine(0,ilun[3][0],ilun[3][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d4(FIL_NULL, (initind>4)?new CSQuickLine(0,ilun[4][0],ilun[4][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d5(FIL_NULL, (initind>5)?new CSQuickLine(0,ilun[5][0],ilun[5][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d6(FIL_NULL, (initind>6)?new CSQuickLine(0,ilun[6][0],ilun[6][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	...
	DialList outerList(firstBox, innerList);
	LaunchDial(1, &outerList);
...
Bool RFullPanelDial::QuickMissionRed()
{
	...
	DialBox d0(FIL_NULL, (initindch>0)?new CSQuickLine(1, ilch[0][0], ilch[0][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d1(FIL_NULL, (initindch>1)?new CSQuickLine(1, ilch[1][0], ilch[1][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d2(FIL_NULL, (initindch>2)?new CSQuickLine(1, ilch[2][0], ilch[2][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d3(FIL_NULL, (initindch>3)?new CSQuickLine(1, ilch[3][0], ilch[3][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d4(FIL_NULL, (initindch>4)?new CSQuickLine(1, ilch[4][0], ilch[4][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d5(FIL_NULL, (initindch>5)?new CSQuickLine(1, ilch[5][0], ilch[5][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d6(FIL_NULL, (initindch>6)?new CSQuickLine(1, ilch[6][0], ilch[6][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);

	// 2. Inner DialList must be an lvalue
	DialList innerList(
		emptyBox,
		(initindch > 0) ? d0 : *ND,
		(initindch > 1) ? d1 : *ND,
	...
	// 3. Outer DialList must be an lvalue
	DialList outerList(firstBox, innerList);
...
Bool RFullPanelDial::QuickViewInit()
{
	...
	// First DialBox must be an lvalue
	DialBox firstBox(FIL_NULL, new EmptyChildWindow, EDGES_NOSCROLLBARS_NODRAGGING);

	DialBox d0(FIL_NULL, (initind>0)?new CViewQList(0, ilun[0][0], ilun[0][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d1(FIL_NULL, (initind>1)?new CViewQList(0, ilun[1][0], ilun[1][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d2(FIL_NULL, (initind>2)?new CViewQList(0, ilun[2][0], ilun[2][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d3(FIL_NULL, (initind>3)?new CViewQList(0, ilun[3][0], ilun[3][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d4(FIL_NULL, (initind>4)?new CViewQList(0, ilun[4][0], ilun[4][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d5(FIL_NULL, (initind>5)?new CViewQList(0, ilun[5][0], ilun[5][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d6(FIL_NULL, (initind>6)?new CViewQList(0, ilun[6][0], ilun[6][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox d7(FIL_NULL, (initind>7)?new CViewQList(0, ilun[7][0], ilun[7][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);

	// Left side list (0 = blue)
	DialList leftList(
		firstBox,
		(initind > 0) ? d0 : *ND,
		(initind > 1) ? d1 : *ND,
	...
	// First DialBox must be an lvalue
	DialBox firstRightBox(FIL_NULL, new EmptyChildWindow, EDGES_NOSCROLLBARS_NODRAGGING);

	DialBox r0(FIL_NULL, (initindch>0)?new CViewQList(1, ilch[0][0], ilch[0][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r1(FIL_NULL, (initindch>1)?new CViewQList(1, ilch[1][0], ilch[1][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r2(FIL_NULL, (initindch>2)?new CViewQList(1, ilch[2][0], ilch[2][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r3(FIL_NULL, (initindch>3)?new CViewQList(1, ilch[3][0], ilch[3][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r4(FIL_NULL, (initindch>4)?new CViewQList(1, ilch[4][0], ilch[4][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r5(FIL_NULL, (initindch>5)?new CViewQList(1, ilch[5][0], ilch[5][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r6(FIL_NULL, (initindch>6)?new CViewQList(1, ilch[6][0], ilch[6][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);
	DialBox r7(FIL_NULL, (initindch>7)?new CViewQList(1, ilch[7][0], ilch[7][1]):NULL, EDGES_NOSCROLLBARS_NODRAGGING);

	// Right side list (1 = red)
	DialList rightList(
		firstRightBox,
		(initindch > 0) ? r0 : *ND,
		(initindch > 1) ? r1 : *ND,
	...
	// Outer DialList using existing Top (DialBox&) and the two lists
	DialList outerList(Top, leftList, rightList);

	LaunchDial(0, &outerList);
...
{	 //First copy over all the info from the CSQuick1::quickdef to globrefs
	...
	// RERUN: Force the flying window to the selected resolution size.
	// If we use Place(), it autosizes to the dialog content, which defaults to small
	// if no template exists.
	int w = winmode_w;
	int h = winmode_h;

	OverLay.loader_art=FIL_PHOTOART;
	CRect placement(0, 0, w, h);
	DialBox flyBoxDial(OverLay.loader_art, new Rtestsh1(NULL, fIsRunning), EDGES_NOSCROLLBARS);
...
bool RFullPanelDial::OnFlyingClosed(RDialog*,int rv)
{
	...
	UpdateSize(); // RERUN return to UI size
...
{
	flybox->PostMessage(WM_COMMAND,IDOK,0);
	return FALSE;
}
...
CFont* RFullPanelDial::OnGetGlobalFont(int fontnum)
{
	...
	if (g_AllFonts[fontnum][1])
	{
		...
		if (currentres>=1280) f = g_AllFonts[fontnum][3];
		else if (currentres>=1024) f = g_AllFonts[fontnum][2];
		else if (currentres>=800) f = g_AllFonts[fontnum][1];
		else f = g_AllFonts[fontnum][0];	
	}
	else if (m_scalingfactor==2) f = g_AllFonts[fontnum][3];
	else f = g_AllFonts[fontnum][0];
	
	if (!f) std::cout << "[RFullPanelDial] OnGetGlobalFont: Returning NULL for fontnum=" << fontnum << " res=" << resolutions[m_currentres] << std::endl;
	return f;
}
...
// load also views now....
	...
	{
		RMessageBox(IDS_MIGALLEY,IDS_INVALIDREPLAYFILE,0,IDS_CONTINUE);	//RJS 03Sep99
		return FALSE;
	}
...
{	
	...
	{
		CString campname = RESSTRING(MIGALLEY) + ": " + LoadResString(IDS_L_SCAMPAIGNSELECT1 + Miss_Man.currcampaignnum);
		AfxGetMainWnd()->SetWindowText(campname);
	}
	else
	{
	...
	// RERUN: Force title bar to update its text
	CMainFrame* main=(CMainFrame*)AfxGetMainWnd();
	if (main) main->m_titlebar.Redraw();
}
...
Bool RFullPanelDial::ReplayView(FullScreen*&fs)
{
	...
	if (!_Replay.ValidReplayFile())
	{
		RMessageBox(IDS_MIGALLEY,IDS_INVALIDREPLAYFILE,0,IDS_CONTINUE);
		return FALSE;
	}
...
Bool RFullPanelDial::StartComms(FullScreen*&fs)
{
	...
	if (_DPlay.StartCommsSession())
		return TRUE;
	RMessageBox(IDS_MIGALLEY,IDS_NOTCONNECTED,0,IDS_CONTINUE);
...
// password check
		...
		if (res)
		{
			...
			if (!res)
			{
				RMessageBox(IDS_MIGALLEY,IDS_NOTCONTACTHOST,0,IDS_CONTINUE);
			}
		}
		else
		{
			RMessageBox(IDS_MIGALLEY,IDS_INCORRECTPASSWORD,0,IDS_CONTINUE);
		}
	}
...
void RFullPanelDial::OnPaint() 
{
	...
	else
	{
		...
		EndPaint(&paintstruct);
		//CDialog::OnPaint(); //RERUN Added anyway TEST TEST TEST	
	}
}
...
{
	RDialog::RMessageBox(IDS_MIGALLEY,mess,0,IDS_OK);
}
...
bool	DPlay::LoadDummySavegame(const char* name)
{
	CString	temp=name;
...
//
//------------------------------------------------------------------------------
bool	Replay::SaveTempSaveGame(const char* name, bool pack)
{
...
Bool	RFullPanelDial::DebriefReplayCheck(FullScreen*&fs)
{
	...
	if (_Replay.badrecord)
	{
		...
		RMessageBox(IDS_MIGALLEY,IDS_BADREPLAY,0,IDS_CONTINUE);
		return FALSE;
	}
