...
//RERUN RFullPanelDial::RFullPanelDial(CWnd* pParent /*=NULL*/)
//RERUN	: RowanDialog(RFullPanelDial::IDD, pParent)
...
RFullPanelDial::~RFullPanelDial()
{
//RERUN	m_pView->m_pfullpane=NULL;
}
...
#else
	int	X = winmode_w/2 - 640/2; //RERUN, more or less the centre
	int	Y = winmode_h/2 - 480/2;
#endif
	if (smackerID)
	{
...
#endif
//RERUN			OpenSmack(smackerID,(int)m_hWnd,X,Y,0,0);					//DAW 09Jul99
		OpenSmack(smackerID, (int)global_hWnd, X, Y, 0, 0); //RERUN

//RERUN		m_timerID=SetTimer(2398,1,NULL);
	}
}
...
{
	bool	closedsmack = true; //RERUN was false;

#if 0 //RERUN until timer for smacker works again
	if (m_timerID) 
	{
		CloseSmack();
		Master_3d.BigWin();
		KillTimer(global_hWnd, m_timerID); //RERUN added global_hWnd
		m_timerID=0;
		closedsmack=true;
	}
#else
	...
	closedsmack = true;
#endif
//RERUN	pdial[0]=pdial[1]=pdial[2]=NULL;
//RERUN	Invalidate();
	m_currentscreen=pfullscreen;
//RERUN	artnum=pfullscreen->resolutions[m_currentres].artwork;
//RERUN	PositionRListBox();
	localnote=NULL;
	...
	if (closedsmack) 
	{
		int	failed=0;
		UpdateSize();
//RERUN		failed=GetCurrentRes();
#if 0 //RERUN
		PAINTSTRUCT paintstruct;
		HDC pDC;
		pDC = BeginPaint(global_hWnd, &paintstruct);
		if (GetDeviceCaps(pDC,BITSPIXEL)<15)
			failed=0;
		if (failed==0)
		{
			//attempt to improve 256 colour palette match?
		}
		EndPaint(global_hWnd, &paintstruct);
...
//RERUN TODO	m_pView->LaunchMap(firsttime);
...
{	
//RERUN	CMainFrame* mainframe = ((CMainFrame*)AfxGetMainWnd());
	Save_Data.SavePreferences();
//RERUN	mainframe->m_doIExist=0;
//RERUN	mainframe->DestroyWindow();
...
//RERUN	LaunchDial(new 	CCredits(IDS_CT_PROGRAMMINGTEAMTITLE),1);
	return TRUE;
}
...
{
//RERUN	DialBox    Top(FIL_NULL,new CQuickView);
	int	ilun[8][2]={{0}};
	int initind=0;
...
bool RFullPanelDial::OnFlyingClosed(/*RERUN RDialog*,*/ int rv)
{
	FullScreen* s=NULL;
...
#ifndef	MIG_DEMO_VER
//RERUN		CMainFrame* main=(CMainFrame*)AfxGetMainWnd();
//RERUN		main->m_titlebar.Redraw();
...
//RERUN				main->m_toolbar1.SetFiltersFromSaveGame();
//RERUN				m_pView->m_mapdlg.Invalidate();
...
//RERUN				main->m_titlebar.Redraw();
				...
				{
					//RERUN if (MMC.Active_Pilots[MMC.playersquadron*24+MMC.playeracnum].status >= IS_DEAD)
					//RERUN 	MainToolBar().OnClickedMissionlog();
					//RERUN else
					//RERUN 	DebriefToolBar().OpenMissionresults();
					LaunchMap(s, true);
				}
			}
			else
...
{
//RERUN	flybox->PostMessage(WM_COMMAND,IDOK,NULL);
	return FALSE;
}
...
//RERUN	LaunchDial(new	CLoad(IDD_SREPLAY,LSD_LOAD,	FIL_VIDEOS_START_DIR,wildcard,selectedfile),0);
...
//RERUN	LaunchDial(new	CLoad(IDD_SAVEREPLAY,LSD_SAVE,	FIL_VIDEOS_START_DIR,wildcard,selectedfile),0);
...
//RERUN	LaunchDial(new CCampBack(IDS_CAMPOBJ0+(whichcamp&0x0f)),2);
	return TRUE;
}
...
{
//RERUN	LaunchDial(new CLockerRoom,0);
	return TRUE;
}
#ifndef	MIG_DEMO_VER
...
//RERUN	LaunchDial(new	CLoad(IDD_LOADFULL,LSD_LOAD,			FIL_SAVEGAMEDIR,wildcard,selectedfile),0);
...
{
//RERUN	LaunchDial(new CSelectService,0);
	return TRUE;
}

Bool RFullPanelDial::SelectSessionInit()
{
//RERUN	LaunchDial(new CSelectSession,0);
	return TRUE;
}
#endif
Bool RFullPanelDial::VisitorsBookInit()
{
//RERUN	LaunchDial(new CVisitorsBook,0);
	return TRUE;
}

Bool RFullPanelDial::ReadyRoomInit()
{
//RERUN	LaunchDial(new CReadyRoom,0);
	return TRUE;
}
#ifndef	 MIG_DEMO_VER
...
{
//RERUN	LaunchDial(new CRadio,0);
	return TRUE;
}

Bool RFullPanelDial::VariantsInit()
{
//RERUN	LaunchDial(new CVariants,0);
	return TRUE;
}					
...
// load also views now....
	...
	{
//RERUN		RMessageBox(IDS_MIGALLEY,IDS_INVALIDREPLAYFILE,NULL,IDS_CONTINUE);	//RJS 03Sep99
		MessageBox(NULL, RESSTRING(INVALIDREPLAYFILE), "Error", MB_OK); //RERUN
		return FALSE;
	}
...
#else
	if (clear)
		SDL_SetWindowTitle(Master_3d.SdlWinInst, RESSTRING(MIGALLEY));
	else
		if (Miss_Man.currcampaignnum != Miss_Man.SCRAMBLECAMPAIGN)
		{
			std::string title = RESSTRING(MIGALLEY) + ": " + 
            LoadResString(IDS_L_SCAMPAIGNSELECT1 + Miss_Man.currcampaignnum);
        	SDL_SetWindowTitle(Master_3d.SdlWinInst, title.c_str());
		}
		else
		{
			std::string title = RESSTRING(MIGALLEY) + ": " +
            LoadResString(CSQuick1::quickmissions[CSQuick1::currquickmiss].missionname);
            SDL_SetWindowTitle(Master_3d.SdlWinInst, title.c_str());
		}
...
Bool RFullPanelDial::ReplayView(FullScreen*&fs)
{
	...
	if (!_Replay.ValidReplayFile())
	{
//RERUN		RMessageBox(IDS_MIGALLEY,IDS_INVALIDREPLAYFILE,NULL,IDS_CONTINUE);
		MessageBox(NULL, RESSTRING(INVALIDREPLAYFILE), "Error", MB_OK); //RERUN
		return FALSE;
	}
...
Bool RFullPanelDial::StartComms(FullScreen*&fs)
{
	...
	if (_DPlay.StartCommsSession())
		return TRUE;
//RERUN	RMessageBox(IDS_MIGALLEY,IDS_NOTCONNECTED,NULL,IDS_CONTINUE);
	MessageBox(NULL, RESSTRING(NOTCONNECTED), "Error", MB_OK); //RERUN
...
// password check
			...
			if (res)
			{
				...
				if (!res)
				{
					//RERUN RMessageBox(IDS_MIGALLEY,IDS_NOTCONTACTHOST,NULL,IDS_CONTINUE);
					MessageBox(NULL, RESSTRING(NOTCONTACTHOST), "Error", MB_OK); //RERUN
				}
			}
			else
			{
				//RERUN RMessageBox(IDS_MIGALLEY,IDS_INCORRECTPASSWORD,NULL,IDS_CONTINUE);
				MessageBox(NULL, RESSTRING(INCORRECTPASSWORD), "Error", MB_OK); //RERUN
			}
		}
	}
...
void	RFullPanelDial::ChangeCamp(int campnum)
{
	...
	if (whichcamp&0x10)
	{
		...
		//RERUN LaunchDial(new CCampBack(IDS_CAMPDESC0+campnum),2);
	}
	else
	if (whichcamp&0x20)
	{
		whichcamp=campnum+0x20;
		//RERUN LaunchDial(new CCampBack(IDS_CAMPOBJ0+campnum),2);
	}
	else
	{
		whichcamp=campnum;
	...
	File_Man.ExistNumberedFile(FileNum(FIL_SMACK_CAMP1INTRO+whichcamp));
		...
		OpenSmack(FileNum(FIL_SMACK_CAMP1INTRO+whichcamp),(int)global_hWnd,-1,-1,0,0); //RERUN, changed m_hWnd to global_hWnd
	}
}		
...
#endif
#endif
	OpenSmack(FileNum(FIL_SMACK_CAMP1INTRO+whichcamp),(int)global_hWnd,-1,-1,0,0);
	return FALSE;
}
...
//RERUN	LaunchDial(new CCampBack(IDS_CAMPDESC0+(whichcamp&0x0f)),2);
	return FALSE;
}
...
#endif
	whichcamp=(whichcamp&0x0f)+0x10;
//RERUN	LaunchDial(new CCampBackEntireWar(IDS_CAMPDESC0+(whichcamp&0x0f)),2);
	return FALSE;
}
...
#endif
	whichcamp=(whichcamp&0x0f)+0x20;
//RERUN	LaunchDial(new CCampBackEntireWar(IDS_CAMPOBJ0+(whichcamp&0x0f)),2);
	return FALSE;
}
...
//RERUN		LaunchDial(new CCampBack(IDS_CAMP5_END00 + index),2);
	...
	}else
	{
		...
		{
			LaunchSmacker(1, MMC.camp_win_anim);
//RERUN			LaunchDial(new CCampBack(IDS_CAMP1ENDGOOD + Miss_Man.currcampaignnum),2);
		}else
		{
			LaunchSmacker(1,MMC.camp_lost_anim);
//RERUN			LaunchDial(new CCampBack(IDS_CAMP1ENDBAD + Miss_Man.currcampaignnum),2);
		}
	}
	return TRUE;
}
...
//RERUN		LaunchDial(new CCampBack(IDS_PLAYERDIED),2);
	return TRUE;
}
Bool	RFullPanelDial::SpecialsInit()
{
		LaunchSmacker(1, MMC.smacker);
//RERUN		LaunchDial(new CCampBack(RESTABLESUM(2,DISPARA_0,MMC.msgwithsmacker)),2);
	return TRUE;
}
...
{
//RERUN TODO	CFiling::LoadGame(selectedfile);
...
Bool	RFullPanelDial::DoLoadGame(FullScreen*&fs)
{
	...
	if (selectedfile=="" || selectedfile==".sav")
		return FALSE;
//RERUN TO DO	if (!CFiling::LoadGame(selectedfile))
		return FALSE;
	Save_Data.lastsavegame = selectedfile;
...
{
//RERUN	RDialog::RMessageBox(IDS_MIGALLEY,mess,NULL,IDS_OK);
	MessageBox(NULL, LoadResString(mess), "Error", MB_OK); //RERUN
}
...
//CLEAN UP CAMPAIGN RECORD FOR NEXT TIME. Should really do it before going in, but Macca is nesh :-) //JIM 30/06/99
	...
	{
//RERUN		RMessageBox(IDS_MIGALLEY,IDS_BADREPLAY,NULL,IDS_CONTINUE);
		MessageBox(NULL, RESSTRING(BADREPLAY), "Error", MB_OK); //RERUN
		return FALSE;
	}
