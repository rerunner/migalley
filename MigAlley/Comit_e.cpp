...
void DirControl::UpdatePackageAircraft(int packnum, int d, int i)
{
	...
	else
	{
		...
		for (char k = 0; k < Profile::MAX_GROUPS; k++)
		{
			...
			{
				ModifyAcInGrp(packnum, 0, k, directives[d].missions[i].group[k].squadron, directives[d].missions[i].group[k].numflights);
			}
			else
			{
...
void DirControl::ListSupplyNodes(int d)
{
	...
	while (currnode->type != LASTONE)
	{
		...
		if ((currnode->attackstats.damagelevel < 100) && ((currnode->currentstores < (3 * maxscore[d] / 4))
			|| (alphastrikesallowed)) && (currnode->status != BEHINDBLUELINES))
		{
			for (char j = 0; j < MAXMISSIONS; j++)
...
void DirControl::ListAirfields(int d)
{
	...
	for (i = 0; i < SupplyTree::AIRFIELDSINWORLD; i++)
	{
		int score = (1 + (int)(SupplyTree::airfieldinfo[i].type)) *  SupplyTree::airfieldinfo[i].capacity;
		if ((SupplyTree::airfieldinfo[i].attackstats.damagelevel < 100) && ((score < (3 * maxscore[d] / 4)) || (alphastrikesallowed)))
		{
			for (j = 0; j < MAXMISSIONS; j++)
			{
				if ((score > directives[d].missions[j].score) && (SupplyTree::airfieldinfo[i].nationality != NAT_BLUE))
				{
...
void DirControl::ProcessFrontLine(int d, SupplyLine* sl)
{
		...
		|| (sl->initiative == BLUEATTACKING)))
	{
		...
		for (i = 0; i<sl->groundbattle.usedteams; i++)
		{
...
bool	DirControl::AllocateTargets(int d, char inputstriketype, bool escortreq)
{
	...
	if ((directives[d].type == SupplyTree::D_RAIL) || (directives[d].type == SupplyTree::D_ROAD))
	...
	{
		//		char striketype;
		int escort, flak, strike, i;
		bool acavailable;
		for (i = 0; i < directives[d].targets; i++)
		{
			char striketype = inputstriketype;
			bool targetalreadyalloc = TargetAlreadyAlloc(directives[d].missions[i].target);

			if ((targetalreadyalloc) && (striketype == UNDECIDEDSTRIKE))
			{
			...
			else
			{
				...
				else
				{
					...
					if ((escort == 0) || (AllocAc(Profile::GRPESCORT, escort, directives[d].type, d)))
					{
						...
						if ((flak == 0) || (AllocAc(Profile::GRPFLAKSUPP, flak, directives[d].type, d)))
						{
					...
					if (acavailable)
					{
						...
						if ((strikeacrequired <= directives[d].strikeac) && (fighteracrequired <= directives[d].fighterac))
						{
...
int DirControl::EscortRequired(SupplyNode*  node)
{
	...
	SWord i = 0;
	COORDS3D& nodeworld = Persons2::ConvertPtrUID(node->associated_item)->World;

	while (SupplyTree::airfieldinfo[i].nationality != NAT_ALIGN)
	{
		if ((SupplyTree::airfieldinfo[i].activeac) && (SupplyTree::airfieldinfo[i].nationality != NAT_BLUE))
		{
			COORDS3D& afworld = Persons2::ConvertPtrUID(SupplyTree::airfieldinfo[i].airfield)->World;
...
bool	DirControl::AllocAc(int grptype, int ac, int type, int d)
{
	...
	char*	choice;
	if ((type == SupplyTree::D_CHOKE) || (type == SupplyTree::D_SUPPLY) || (type == SupplyTree::D_AIRFIELDS))
		choice = jetfirst;
	else
		choice = propfirst;

	int i;
	for (i = 0; i < squadavail; i++)
	{
	...
	if (firsttype != ACT_NOTSET)
	{
		...
		else if ((firsttype == ACT_PROP) && (choice == propfirst))
		{
			int acreq = ac;
			for (i = 0; i < MAX_DIRGROUPS; i++)
				acreq += 4 * tmpgroup[i].numflights;
...
int	DirControl::StrikeTypeRequired(int d, int i)
{
	...
	{
		if (((directives[d].missions[i].score >= (3 * maxscore[d] / 4))
			|| (CloseToChina(5, directives[d].missions[i].node))) && (alphastrikesallowed))
			type = ALPHASTRIKE;
		else if ((directives[d].missions[i].score > (maxscore[d] / 2))
...
bool		DirControl::CloseToChina(int node_num, SupplyNode*  node)
{
	...
	while ((currnode->forward != NULL) && (cnt < node_num) && (currnode != node))
	{
		cnt++;
...
void	DirControl::MakeMiniCampaignMission()
{
	...
	if (Miss_Man.currcampaignnum == MissMan::F86_CAMPAIGN)
	{
		...
		if (pack >= 0)
		{
			...
			bool preloaded =
				Todays_Packages.PreLoad(pack, FIL_BARCAP, NULL, false, true);
...
bool	DirControl::ModifyRedoIfNecessary(int currpack)
{
	...
	for (wave = 0; wave<Profile::MAX_WAVES; wave++)
	{
		for (group = 0; group<Profile::MAX_GROUPS; group++)
		{
			if ((Todays_Packages[pack][wave][group].uid) && (redopossible))
			{
...
void	DirControl::AircraftAlreadyUsed(int excludedpack, bool countall)
{
	...
	for (pack = 0; pack<Profile::MAX_PACKS; pack++)
	{
		if ((Todays_Packages[pack].packagetarget != UID_NULL)
			&& (pack != excludedpack)
			&& ((Todays_Packages[pack].redo) || (countall)))
		{
			...
			for (wave = 0; wave<Profile::MAX_WAVES; wave++)
			{
				for (group = 0; group<Profile::MAX_GROUPS; group++)			  //RDH 22/06/99
				{
