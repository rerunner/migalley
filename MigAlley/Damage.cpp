...
CDamage::CDamage(int sgt,int uid) //RERUN ,CWnd* pParent /*=NULL*/)
//RERUN	: RowanDialog(CDamage::IDD, pParent)
{
	//{{AFX_DATA_INIT
...
//	}
	//}}AFX_DATA_INIT
//RERUN	SetProjSpecific(-1,sgt,uid);
...
{
	if ((FormationInfo.duty&DUTYMASK)==DC_WW)
		return (currtarget->MigStatus.LaunchFlags.deadtime!=0);
	else
	if (!currtarget)
		return	true;
	else
	if (currtarget->MigStatus.LaunchFlags.size>=FORMATIONSIZE)
	{
		FormationItemPtr curr=*currtarget;
		if (curr->MigStatus.LaunchFlags.deadtime && !curr->leader)
	...
	else
	{
		...
		if (SGT==UID_NULL)
			SGT=currtarget->uniqueID.bitfieldUid.count;
...
bool	FormationItem::SelectedElement(ItemPtr currtarget,int counter/*=0*/,Coords3D* coords/*=&despos*/)
{
	...
	if (currtarget->MigStatus.LaunchFlags.size>=FORMATIONSIZE)
	{
		...
		*coords=curr->World;
		return (curr->MigStatus.LaunchFlags.deadtime==0);
	}
	else
	{
...
void	Item::ScoreSimpleItemKilled(bool itsdead,AirStrucPtr killer)
{
	...
	if (killer && killer->MigStatus.LaunchFlags.size!=AIRSTRUCSIZE)
		killer=NULL;
	...
	if (killer)
	{  
		killernat=killer->AutoMoveInfo.nationality;
		killerisgroundsite=false;
		killerisplayer=killer->AcIsPlayer()!=NULL;
		if (killer->ai.PilotSkills.pilotnum<ai_info::PROPER_PILOT_MAX)
		{
			killerpilot=killer->ai.PilotSkills.pilotnum;
	...
	if (itsdead)
	{
		if (MigStatus.LaunchFlags.size>=MOBILESIZE)
			if (MigStatus.LaunchFlags.size==AIRSTRUCSIZE)
			{
				AirStrucPtr deader=*this;
				Nationality deadernat=deader->AutoMoveInfo.nationality;
				bool deaderisplayer=deader->AcIsPlayer()!=NULL;
				SquadSelect	deadersquad;
				int deaderpilot=-1;
				if (deader->ai.PilotSkills.pilotnum<ai_info::PROPER_PILOT_MAX)
				{
					deaderpilot=deader->ai.PilotSkills.pilotnum;
				...
				if (killer && killernat==NAT_BLUE)
				{
					...
					{
						if (	killer->FormationInfo.duty==DUTYMIGCAP ||  killer->FormationInfo.duty==DUTYBARCAP )
							debrief.barcapkills++;
				...
				else
				{
					...
					if (deadernat==NAT_BLUE)
					{
						...
						if (	deader->FormationInfo.duty==DUTYMIGCAP || deader->FormationInfo.duty==DUTYBARCAP)
	...
	//TEMPCODE JIM 18/05/99 			switch (Persons2::getbandfromUID(uniqueID.bitfieldUid.count))
	//TEMPCODE JIM 18/05/99 			{
	//TEMPCODE JIM 18/05/99 
	//TEMPCODE JIM 18/05/99 
	//TEMPCODE JIM 18/05/99 
	...
	//TEMPCODE JIM 18/05/99 		}
			if (counttype==Debrief::TARG_MAX)
			{
				if (killer && killer->IsUsingPilotedAcLog() && killer->ai.homebase!=this && SGT!=killer->ai.homebase->uniqueID.bitfieldUid.count)
				{
...
//
//////////////////////////////////////////////////////////////////////
MAKEFIELD(UniqueID,UID_NULL,IllegalSepID); // RERUN now global scope
...
////	SupplyTree::ClearAllStatuses();
	// RERUN moved to global scope MAKEFIELD(UniqueID,UID_NULL,IllegalSepID);
					...
					{	//if had some effect on total damage then need comparator.	
						...
						for (int i=0;i<IllegalBAND;i++)
							if (i<WayPointBAND || i>WayPointBANDEND)
							{
								...
								if (p2 && p2->MigStatus.LaunchFlags.size!=WAYPOINTSIZE)
								{
									ItemPtr p=*p2;
									if (i==SGT)
										weights.AddWeightTotal((p->shape),UniqueID(i),false);
...
//RERUN	UByte	retval = 0;
//RERUN	return retval;
	return *deadittfirstbyteptr; //RERUN: return does not matter anymore here
}
...
void	ArtInt::FakeMissionSuccess()
{
	for (AcArrayItterator itt;itt.Next();)
		if (itt)
		{
			if (itt->AutoMoveInfo.nationality==NAT_BLUE)
				if (!itt->MigStatus.LaunchFlags.deadtime)
					if (itt->AutoMoveInfo.movecode!=AUTO_FAC)
...
{	//called to decide wether a mission was a success...
	...
	int migskillfactor=	( MMC.migsquad[0].generalskill*20+MMC.migsquad[0].morale+MMC.migsquad[0].agglevel*50+MMC.migsquad[0].maxevent*10
						...
						/300;
	if (AutoMoveInfo.nationality==NAT_BLUE)
		if (waypoint && waypoint->wpname && !Math_Lib.rnd(10+waypoint->wpname*8+ai.PilotSkills.flyingskill*3-migskillfactor) && !AcIsPlayer())
		{
			ScoreSimpleItemKilled(true,NULL);
		}
		else
		if (startwp && startwp->wpname && !Math_Lib.rnd(10+startwp->wpname*8+ai.PilotSkills.flyingskill*3-migskillfactor) && !AcIsPlayer())
		{
			ScoreSimpleItemKilled(true,NULL);
		}
		else
		if (startwp)
		{
			...
			while (targwp && targwp->wpnum)	  //SQ_B29=6 SQ_F86A=0
				...
				if ((FormationInfo.duty&ACTIONCODE)==DA_CAP)				//H=2						//H=5
				{		//F86=rnd(HARDER*3+7)/(HARDER*3)	13/6 =6x0,6x1,1x2 =>8/13	22/15==> 15x0,7x1 = 7/22 =0.31
					...
					if (numkills)
					{
						...
						if (targwp)
						{
							WayPointPtr patrolwp=targwp->NextWP(uniqueID.bitfieldUid.count);
							...
							if (target==NULL || target==targwp)
							{
								...
								if (topleader)
								{
									patrolwp=targwp->NextWP(topleader->uniqueID.bitfieldUid.count);
							...
							if (target)
								if (target->MigStatus.LaunchFlags.size>=ITEMSIZE && ItemPtr(target)->SGT)
									targuid=ItemPtr(target)->SGT;
								else
									targuid=target->uniqueID.bitfieldUid.count;
						...
						if (targuid!=UID_NULL)
						{
							...
							if (sn)
								...
								switch(FormationInfo.duty&DUTYMASK)
								{
								case DC_CAS:
										...
										while (wp && wp->wpname==WPNAME_Target)
										{
											if (wp->action==wpstartloopno && wp->target)
												if (wp->target->SGT)
													...
													t1=wp->target->uniqueID.bitfieldUid.count;
											if (wp->action==wpstoploopchkno && wp->target)
												if (wp->target->SGT)
													t2=wp->target->SGT;
												else
													t2=wp->target->uniqueID.bitfieldUid.count;	
											wp=wp->NextWP(uniqueID.bitfieldUid.count);
										}
									...
									if (targuid)
									{
										...
										while (--numkills>=0)
										{
											...
											forever
											{
												...
												if (trg && trg->MigStatus.LaunchFlags.size==ITEMSIZE)
												{
													target=*trg;
...
void		TeamDef::DiagnosticOutput(const char* intro)
{	 
}


void	SupplyLine::DiagnosticOutput(int type,const char* intro)
{
}
